/* =========================================================================
   ...
   Тор проецируется из 3D в 2D: кольцо лежит плашмя (наклон Rx — вид сверху)
   и вращается в своей плоскости (Rz), как монета на столе, по часовой
   стрелке. Надпись выгравирована на внешней стороне кольца и видна только
   на обращённой к зрителю половине. Палитра следит за темой страницы.
   ========================================================================= */
...
export function initRing3d(id = 'ring3d') {
  ...
  function draw(now) {
    const t = now * 0.001;
    const spin = -t * 0.55;                           // кольцо лежит: вращение в своей плоскости (по часовой)
    const tilt = 0.85 + Math.sin(t * 0.45) * 0.07;    // лежит плашмя (вид сверху) + лёгкое покачивание
    const py = cy + Math.sin(t * 0.7) * h * 0.014;    // плавное парение

    const sc = Math.cos(spin), ss = Math.sin(spin);
    const tc = Math.cos(tilt), ts = Math.sin(tilt);

    // Кольцо лежит (Rx — наклон кольца плашмя, вид сверху) и вращается в своей
    // плоскости (Rz) — как монета на столе, по часовой стрелке.
    const rotW = (x, y, z) => {
      const x1 = x * sc - y * ss;    // Rz: вращение внутри плоскости кольца
      const y1 = x * ss + y * sc;
      const z1 = z;
      const y2 = y1 * tc - z1 * ts;  // Rx: наклон "лежит" / перспектива сверху
